-- ar_subs.http: async HTTP layer with rate-limit header capture and
...
function M.request_async(url, opts, on_done)
  ...
  if mp.command_native_async == nil then
    if not _warned_sync_fallback then
      mp.msg.warn("ar_subs: mp.command_native_async unavailable, using sync fallback")
    ...
    if is_rate_limited(json, http_code, headers) and backup_key ~= "" and backup_key ~= api_key then
      mp.msg.info("ar_subs: rate limited, retrying with backup key")
